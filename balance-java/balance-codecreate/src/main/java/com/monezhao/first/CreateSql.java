...
public class CreateSql {
    ...
    public static void create(String createTableName) {
        ...
        } else if (SQLSERVER.equals(CodeUtil.databaseType)) {
            resolveSqlserver(stringBufferAll, hashMap);
        }
    ...
    private static void resolveMysqlTable(ArrayList<TableObject> list, StringBuffer stringBufferMid,
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (list.size() != i + 1) {
                ...
                // mysql，主键为整数型，数据库自增长
                if (isNumberPk) {
                    stringBufferFoot.append(") ENGINE=InnoDB AUTO_INCREMENT=1001 DEFAULT CHARSET = utf8mb4" +
                    ...
                    stringBufferFoot.append(
                            ") ENGINE=InnoDB DEFAULT CHARSET = utf8mb4" +
                                    "  COLLATE = utf8mb4_general_ci COMMENT='" + tableObject.getTableNameCn() + "';\r\n");
                }
            }
        }
    }
    ...
    private static void resolveSqlserver(StringBuffer stringBufferAll, HashMap<String, ArrayList<TableObject>> hashMap) {
        for (Object tableName : hashMap.keySet()) {
            // 读到的表名对象list.
            ArrayList<TableObject> list = hashMap.get(tableName);
            // T_SYS_CODE_TYPE变成SYS_CODE_TYPE
            ...
            stringBufferHead.append("/**【" + tableName + "】开始**/" + "\r\n");
            stringBufferHead.append("drop table " + tableName + ";" + "\r\n");
            stringBufferHead.append("CREATE TABLE [dbo].[" + tableName + "]\r\n");
            stringBufferHead.append("(" + "\r\n");

            resolveSqlserverTable(list, stringBufferMid, stringBufferFoot, (String) tableName);
    ...
    private static void resolveSqlserverTable(ArrayList<TableObject> list, StringBuffer stringBufferMid,
                                              StringBuffer stringBufferFoot, String tableName) {
        // 主键字段
        String pkColumn = "";
        for (int i = 0; i < list.size(); i++) {
            TableObject tableObject = list.get(i);
            ...
            if ("字符型".equals(tableObject.getDataType())) {
                ...
                stringBufferMid.append("datetime2 ");
            } else if ("大文本".equals(tableObject.getDataType())) {
                // TinyText、Text、MediumText、LongText的大小区别(字节):255、65k、16M、4G。
                stringBufferMid.append("nvarchar(max) ");
            } else if ("大文件".equals(tableObject.getDataType())) {
                // TinyBlob、Blob、MediumBlob、LongBlob的大小区别(字节):255、65k、16M、4G。
                stringBufferMid.append("blob ");
            }
            ...
            if (list.size() != i + 1) {
                ...
                if (!CommonUtil.isEmptyStr(pkColumn)) {
                    stringBufferMid.append("," + "\r\n");
                    stringBufferMid.append("    " + "PRIMARY KEY CLUSTERED ([" + pkColumn + "])" + "\r\n");
                }
                // 建表语句结束
                stringBufferFoot.append(
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            stringBufferFoot.append("\n\nEXEC sp_addextendedproperty\n" +
                    "'MS_Description', N'" + tableObject.getColumnNameCn() + "',\n" +
                    "'SCHEMA', N'dbo',\n" +
                    "'TABLE', N'" + tableName + "',\n" +
                    "'COLUMN', N'" + tableObject.getColumnNameEn() + "'\n" +
                    "GO ");
        }
    }
}
