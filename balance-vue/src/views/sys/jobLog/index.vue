<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="是否正常执行" prop="status" align="center"><template slot-scope="scope"><span v-html="formatDictText(dicts.yesOrNo,scope.row.status)" /></template></el-table-column>
    ...
    <el-dialog title="定时任务执行日志" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" :disabled="dialogStatus==='view'" label-position="right" label-width="auto">
        ...
        <el-form-item label="是否正常执行" prop="status"><el-select v-model="temp.status" placeholder="是否正常执行"><el-option v-for="(item, index) in dicts.yesOrNo" :key="index" :label="item.content" :value="item.value" /></el-select></el-form-item>
        <el-form-item label="异常信息" prop="exceptionInfo"><el-input v-model="temp.exceptionInfo" /></el-form-item>
        <el-form-item label="开始时间" prop="startTime"><el-date-picker v-model="temp.startTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" /></el-form-item>
        <el-form-item label="结束时间" prop="stopTime"><el-date-picker v-model="temp.stopTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" /></el-form-item>
      </el-form>
