<template>
  <el-dialog :title="title" fullscreen :visible.sync="dialogProcessDefinitionIdentityLinkInChild" width="75%">
    <el-button-group style="padding-bottom: 10px">
      <el-button v-permission="'flowable:processDefinitionIdentityLink:save'" icon="el-icon-plus" type="primary" class="filter-item" @click="btnCreate">新增</el-button>
    </el-button-group>
    <el-table
      :data="records"
      border
    ...
    >
      ...
      <el-table-column label="授权类型" prop="identityType" align="center">
        <template slot-scope="scope">
          <span v-html="formatDictText(dicts.identityType,scope.row.identityType)" />
        </template>
      </el-table-column>
      ...
      >
        <el-form-item label="授权类型" prop="identityType">
          ...
          <el-select v-model="temp.identityType" placeholder="授权类型">
            <el-option
              v-for="(item, index) in dicts.identityType"
              :key="index"
              :label="item.content"
              :value="item.value"
            />
