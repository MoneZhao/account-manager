<template>
  <el-dialog title="执行任务" :visible.sync="dialogExcuteTaskVisibleInChild" :fullscreen="false">
    <el-tabs v-model="activeName" type="border-card">
      <el-tab-pane v-if="generateTaskFormVisible" name="taskForm" label="任务表单">
        <fm-generate-form ref="generateFormTask" :data="taskFormJson" :value="taskFormData" />
      </el-tab-pane>
    ...
    <div slot="footer" class="dialog-footer" style="padding:0 15px 0 15px">
      ...
      <el-button
        v-if="$store.getters.sysUser.userId==='admin'||$store.getters.sysUser.userId===startUserId"
        icon="el-icon-close"
        type="primary"
        @click="doStop"
      >终止
