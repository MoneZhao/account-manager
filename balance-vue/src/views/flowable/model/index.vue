  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.modelCategory" placeholder="模型类别" class="filter-item">
        <el-option
          v-for="(item, index) in dicts.processCategory"
          :key="index"
          :label="item.content"
          :value="item.value"
        />
      ...
      <el-button-group>
        <el-button
          v-permission="'flowable:model:save'"
          icon="el-icon-plus"
          type="primary"
          class="filter-item"
          @click="btnCreate"
        ...
        <el-button
          v-permission="'flowable:model:import'"
          icon="el-icon-plus"
          type="primary"
          class="filter-item"
          @click="btnImport"
    ...
    >
      ...
      <el-table-column label="模型类别" prop="category" align="center">
        <template slot-scope="scope">
          <span v-html="formatDictText(dicts.processCategory,scope.row.category)" />
        </template>
      </el-table-column>
        ...
        <template slot-scope="{row}">
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item
                v-permission="'flowable:model:update'"
                icon="el-icon-edit"
                divided
              ...
              <el-dropdown-item
                v-permission="'flowable:model:saveModelEditor'"
                icon="el-icon-edit"
                divided
                @click.native="btnUpdateModel(row)"
              >流程设计
              </el-dropdown-item>
              <el-dropdown-item
                v-permission="'flowable:model:delete'"
                icon="el-icon-delete"
                divided
              ...
              <el-dropdown-item
                ...
                icon="el-icon-delete"
                divided
                @click.native="btnDelete(row.id,true)"
              >删除包含历史
              </el-dropdown-item>
              <el-dropdown-item
                v-permission="'flowable:model:deploy'"
                icon="el-icon-edit"
                divided
                @click.native="btnDeploy(row.id)"
              >部署
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
      ...
      >
        ...
        <el-form-item label="模型类别" prop="category">
          <el-select v-model="temp.category" placeholder="模型类别">
            <el-option v-for="item in dicts.processCategory" :key="item.value" :label="item.content" :value="item.value" />
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="流程导入" :visible.sync="dialogImportVisible">
      ...
      >
        ...
        <div slot="tip" class="el-upload__tip">只能上传.bpmn20.xml,.bpmn文件，且不超过512K</div>
      </el-upload>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  beforeCreate() {
    this.getDicts('processCategory').then(({ data }) => { this.dicts = data })
  },
  created() {
    this.list()
  },
